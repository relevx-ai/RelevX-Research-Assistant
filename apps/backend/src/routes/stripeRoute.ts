    ...
    async (request, rep) => {
      ...
      if (event.type === "checkout.session.completed") {
        ...
        if (metadata && metadata.planId && metadata.userId) {
          const planDoc = await db.collection("plans").doc(metadata.planId).get();
          if (!planDoc.exists) {
          ...
          if (!userDoc.exists) {
            ...
            if (userData.billing.stripeCustomerId !== customerId) {
              app.log.error("Users customer id does not match the stripe session customer id");
            }
            else {
              const newUserData = {
                ...userData,
            ...
            app.log.info(`checkout.session.completed: ${JSON.stringify(session)}`);
          }
        }
      }
